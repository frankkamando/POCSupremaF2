...
{    
    class Util
    {
        ...
        public static string GetBase64ImageWSQ(Bitmap Image)
        {
            try
            {
                WsqEncoder wsqEnconder = new WsqEncoder();
                var probeFinger = wsqEnconder.EncodeGdi(Image, 0.75F);
                string data = Convert.ToBase64String(probeFinger);
                return Convert.ToBase64String(probeFinger);
        ...
        public static string GetBase64frombyte(byte[] byteImage)
        {
            try
            {
